...
| Entity unique ID | Optional full override of the climate `unique_id`. Leave empty to use `{host}_{unit}`. Set this to the legacy unique ID to keep existing entities. | `{host}_{unit}` |
